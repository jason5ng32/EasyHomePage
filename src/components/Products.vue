            ...
            <div v-else class="grid gap-5 md:grid-cols-3">
                <article v-if="featuredProduct" class="reveal-up overflow-visible rounded-[2rem] border bg-card p-4 elevated md:col-span-3 md:p-6">
                    <div class="grid gap-6 md:grid-cols-[0.88fr_1.12fr] md:items-center">
                        <div class="p-3 md:p-4">
                            <Badge class="h-auto rounded-full bg-accent text-accent-foreground">{{ section.featuredLabel }}</Badge>
                        ...
                        <div class="media-frame aspect-16/10 rounded-3xl md:translate-x-6 md:translate-y-6 md:-rotate-2 md:shadow-2xl">
                            <img :src="featuredProduct.cover" :alt="featuredProduct.title" decoding="async">
                        </div>
                    </div>
                </article>
                ...
                >
                    <div class="media-frame aspect-[16/10]">
                        <img :src="product.cover" class="transition duration-500 group-hover:scale-105" :alt="product.title" loading="lazy" decoding="async">
                    </div>
                    <div class="p-5">
                        <div class="flex items-start justify-between gap-4">
                            ...
                            <a
                                ...
                                :aria-label="`${section.openLabel} ${product.title}`"
                            >
